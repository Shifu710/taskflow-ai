...
const sections = [
  ...
  ["Architecture diagram", "Next.js App Router frontend calls FastAPI. FastAPI owns auth, RBAC, workflow APIs, LangGraph runtime, tool gateway, usage logs, SSE run streams, and optional Celery worker dispatch. SQLite is quick-demo mode; PostgreSQL plus Alembic is the production data path."],
  ...
  ["Provider strategy", "ModelGateway supports server-side DeepSeek, Qwen, and OpenAI-compatible providers when keys are configured. Missing keys keep the app working with honest demo-local model responses."],
  ...
  ["Known limitations", "Public demo tools use seeded demo data. External write tools are simulated in public demo mode. SQLite is quick-demo mode. Celery/Redis worker mode and Alembic migrations are wired, but active scheduled trigger execution and real Langfuse trace sending still need production hardening. Managed PostgreSQL on Render is the production data path and is applied via render.yaml."],
  ["Production deployment notes", "Frontend: https://taskflow-ai-seven-eosin.vercel.app (Vercel). Backend: deploy services/api with render.yaml for managed Postgres, TASKFLOW_USE_CELERY=false, AI_PROVIDER=demo-local, and rotated TASKFLOW_JWT_SECRET / TASKFLOW_WEBHOOK_SECRET. Set FRONTEND_ORIGINS to the Vercel URL and NEXT_PUBLIC_API_BASE_URL to the Render /api/v1 base. Langfuse and real provider keys stay server-side only."]
];
